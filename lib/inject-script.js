...
var injectScript = function(source) {
	...
	var jsEscape = function(str) {
		...
		do {
			c = r.exec(str);
			result += (c 
				? (str.substring(l, r.lastIndex-1) + "\\x" + c[0].charCodeAt(0).toString(16))
				: (str.substring(l)));
		}
	...
	if (bFunction) {
		...
		for (var i = 1, n=arguments.length, arg; i < n; i++) {
			var raw = arguments[i];
		...
		while (id.length < 16) {
			id += String.fromCharCode(((!id.length || Math.random() > 0.5)
				? 0x61 + Math.floor(Math.random() * 0x19)
				: 0x30 + Math.floor(Math.random() * 0x9 )));
		}
